# VeilNode 技术说明

VeilNode Suite 的核心原则是：所有平台客户端只负责界面和系统集成，真正的加密、封装、协议兼容和载体适配统一走 `veil-core`（当前 Python 实现目录为 `veil_core/`）。

## 命名

- 项目名：`VeilNode`
- 套件名：`VeilNode Suite`
- 核心库：`veil-core`
- 母程序 / 节点生成器：`veil-factory`
- 命令行节点客户端：`veil-node`
- 图形客户端：`veil-gui`
- 消息协议：`veil-msg`
- 内部消息格式：`.vmsg`
- 身份文件：`.vid`
- 密钥分片文件：`.vkp`
- root keypart 种子：`.vkpseed`
- 节点包：`.vpkg`

项目描述：

```text
VeilNode — offline envelope encryption for ordinary carrier files in high-review-pressure environments.
```

这是工程目标，不是绝对安全承诺。项目使用 low-signature、metadata minimization、local privacy hygiene、engineering risk score 等准确表述，不承诺绝对无法识别，也不承诺免于取证分析。

## 核心分层

- `crypto`：Argon2id、HKDF、AES-GCM、XChaCha20-Poly1305 兼容层、哈希和编码。
- `compression`：输入文件/文件夹的标准化打包与恢复。
- `chunks`：payload 分片、基于密钥的乱序和重组。
- `padding`：随机 padding 与 bucket padding。
- `container`：载体生成、嵌入、提取、容量评估和可打开性验证。
- `identity` / `contacts`：本地身份、`.vid` 公钥身份和联系人。
- `message`：`veil-msg` 构建、解密和事务式 auth_state 消费。
- `nodepkg`：`.vpkg` 导出、导入和检查。
- `protocol`：协议版本与兼容性检查。
- `profile`：`dev`、`balanced`、`hardened` 安全档案。
- `platform`：`SecureStore`、`DeviceBinding`、`FileProvider` 平台边界。
- `diagnostics` / `repair`：doctor、audit、capacity、repair、migrate。
- `api`：给 CLI、GUI 和移动端调用的统一 API。Linux 与 NAS GUI 不再作为发行目标；这些系统使用 CLI。
- `strategy`：Adaptive Envelope Policy Engine，负责特征提取、合法策略生成、dry-run 选择、工程风险评分、dataset 收集和 heuristic ranker。

## 文件格式

`.vid` 只包含节点 ID、名称、公钥、创建时间和格式类型，不包含私钥。

`.vkp` 是 v1 每条消息独立生成的密钥分片文件，包含密封记录、协议元数据、载体验证信息和完整性哈希。它不是完整密钥，不能单独解密。

`.vkpseed` 是 v2 长期离线 root keypart seed。文件本身通过 Argon2id + AES-GCM 密码保护，`inspect` 只显示 kind、version、created_at、KDF 参数和 fingerprint，不显示 seed。

v2 每条消息的派生关系：

```text
vkp_i = HKDF(root_vkp, salt=msg_id, info="veil-vkp-v2" || file_hash || receiver_id)
password_key = Argon2id(password, salt=message_salt, profile_params)
message_key = HKDF(vkp_i || password_key, salt=message_salt, info="veil-message-key-v2")
```

`msg_id` 和 `message_salt` 每条消息随机。`root_vkp`、`vkp_i`、`message_key` 都不会写入载体。v2 会在载体内放置接收方派生 `vkp_i` 所需的公开 per-message 恢复元数据，从而免去每条消息传递 `.vkp`。

`.vpkg` 是固定客户端导入的节点包，包含公开身份、加密私钥材料、Profile、联系人、adapter 列表和完整性标签。桌面端和移动端围绕固定客户端 + `.vpkg` 模型保持节点迁移一致性。

## 离线信封加密内核 v2.2

`crypto_core_version = "2.2"` 是离线信封加密内核版本，不是 VeilNode Suite 包版本，也不是 `root_file_version`。

v2.2 root 文件使用 `root_file_version: 2`，包含 `root_id`、`root_epoch`、`fingerprint`、`prev_fingerprint`、`status`、`created_at`、`rotated_at`、`valid_from`、`valid_until`。seed 必须加密保存，metadata 通过 MAC/AEAD 检查，`inspect` 不显示 secret。

v2.2 普通模式消息 outer metadata 只保留恢复提示：

```json
{
  "protocol_family": "veil-offline-envelope",
  "crypto_core_version": "2.2",
  "metadata_schema_id": "...",
  "encrypted_metadata_len": 0,
  "encrypted_metadata_nonce": "...",
  "encrypted_metadata_tag": "...",
  "root_hint": "...",
  "receiver_hint": "..."
}
```

inner encrypted metadata 包含 `msg_id`、`message_salt`、`file_hash`、`receiver_id`、`root_id`、`root_fingerprint`、`root_epoch`、KDF 参数、flags 和 payload 恢复数据。metadata 不得包含 `root_vkp`、`vkp_i` 或 `message_key`。

v2.2 low-signature 写入载体时会把同一组 outer 字段转换为短键 JSON，读取时再在内存中还原。这样 `crypto_core_version` 和 `veil-offline-envelope` 不会作为固定明文留在 carrier 字节里。

root 生命周期：

- `active`：可加密和解密。
- `retired`：默认不可加密，但可解密历史消息。
- `revoked`：默认不可加密/解密；仅 `open --allow-revoked-root` 可显式恢复历史消息。

本地防重放使用 `<home>/state/msg_seen.db`。只有解密、hash 校验和输出提交都成功后才写入 seen 记录。`--no-replay-check` 只用于测试和本地调试。

Shamir 备份使用 GF(256) threshold shares；单个 share 不泄露 root seed，少于 threshold 无法恢复，混入不同 root 的 share 必须失败。

decoy 模式在同一 carrier 内保存独立认证的真实/诱饵 payload。它是抗胁迫辅助，不是数学上完美的可否认加密。

low-signature mode 需要显式启用，目标是降低固定工程特征、最小化 metadata、改善本地隐私卫生和格式保真度。carrier audit/compare/profile 只输出本地工程风险评分，不证明载体一定不会被判定为异常。

## Adaptive Envelope Policy Engine

自适应策略引擎不是新加密算法，也不会让模型生成密码学规则。它只选择 envelope policy：

```json
{
  "policy_version": 1,
  "crypto_core_version": "2.2",
  "carrier_format": "zip",
  "embed_strategy": "zip_stored_member",
  "chunk_profile": "mixed_medium",
  "padding_profile": "mimic-carrier",
  "metadata_layout": "encrypted_split",
  "locator_strategy": "keyed_locator_v2",
  "low_signature": true,
  "risk_budget": 0.25
}
```

选择流程：

1. 提取 carrier / payload features。
2. 从 registry 生成合法候选 policy。
3. 可选使用本地 `model.json` 排序。
4. 在临时目录 dry-run embedding。
5. 必须执行 `verify-carrier`。
6. 执行 carrier audit / compare。
7. 执行 strategy score 和 fixed-signature scan。
8. 选择通过验证且分数最低的候选。
9. 自动清理临时文件。

评分只代表本地工程风险，分数越低越好。parser 失败就是高风险；固定明文标识会加 penalty。它不是不可检测证明，也不针对具体分析工具做绕过。

policy / dataset / model 都不能包含 root seed、`root_vkp`、`vkp_i`、`message_key`、password 或私钥材料。dataset 可以保存非敏感统计特征和短 hash prefix。第一版模型是可读、可版本化的 `heuristic_ranker`；以后可以扩展，但 verifier/audit/compare/score 仍然必须执行。

主要命令：

```bash
veil-node strategy features --carrier cover.zip --payload secret.txt --json
veil-node strategy generate --carrier cover.zip --payload secret.txt --count 50 --json
veil-node strategy select --carrier cover.zip --payload secret.txt --count 50 --json
veil-node strategy score --before cover.zip --after message.zip --policy selected.policy.json --json
veil-node strategy scan-signature --input message.zip --json
veil-node strategy collect --samples-dir samples --payloads-dir payloads --out dataset.jsonl
veil-node strategy train --dataset dataset.jsonl --out model.json
veil-node strategy model inspect --in model.json
```

`.vmsg` 是 Veil 内部消息封装格式，用于跨平台交换、调试和回归测试。它不是伪装成图片、视频、ZIP 的普通载体。

## 解密事务

一次性认证状态必须在“成功之后”消费：

1. 读取 auth_state。
2. 解密 v1 keypart 记录，或用 `.vkpseed` 派生 v2 message key。
3. 恢复 file_key。
4. 从载体提取 payload。
5. 解密 manifest 和内容。
6. 校验 archive hash。
7. 写入临时 staging 目录。
8. 原子提交输出。
9. 只有全部成功后才消费 auth_state。

`verify-only` 会在写出和消费认证状态之前停止。

## 第一批稳定载体

- ZIP：stored member。
- PDF：增量 stream object。
- MP4：`free` box。
- PNG：合法 ancillary chunk。
- WAV：未知 RIFF chunk。
- BMP / 7z：尾部附加兼容路径。
- `.vmsg`：内部裸封装。

JPEG 适合作为输入 payload；核心载体优先选择 ZIP/PDF/MP4/PNG 等字节保持能力更明确的格式。

## 测试向量

```bash
veil-node test-vector
```

该命令验证 v1 XChaCha20-Poly1305 兼容向量和 v2 root keypart 派生向量，是 Rust、Swift、Kotlin、WASM 绑定保持跨平台兼容的回归锚点。
